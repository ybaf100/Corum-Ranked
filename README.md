...
- `v0.4.0-alpha.11`의 Debug Bot Match는 개발용이지만, 테스트 기간에는 일반 PvP와 동일하게 MMR·Ranked Score·Placement·W/L에 반영됩니다.
...
> v0.4.0-alpha.11 policy: the Ranked allowlist checks currently enabled + loaded mods only. Installed-but-disabled mods do not block Ranked; required mods such as CBF must still be active.
