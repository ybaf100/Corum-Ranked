...
최신 alpha.16 변경 파일·검증 결과는
[`docs/v0.4.0-alpha.16-report.md`](docs/v0.4.0-alpha.16-report.md)에 정리되어 있습니다.


> v0.4.0-alpha.16 policy: the Ranked allowlist checks currently enabled + loaded mods only. Installed-but-disabled mods do not block Ranked; required mods such as CBF must still be active.


## alpha.16 song / ban / difficulty fixes
The Ranked client now uses the vanilla CustomSongWidget download path, protects the active match scene stack from repeated back/quit input, and places the main-menu Ranked shortcut in the upper-right.
